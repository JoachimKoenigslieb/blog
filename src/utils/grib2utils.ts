...
export const decodeGRIB2File = function (buffer: ArrayBuffer) {
  let gribByteIndex = 0;
  let gribFiles: GRIB2[] = [];
  let gribFileBuffers: ArrayBuffer[] = [];

  // Separate GRIB buffers
  while (gribByteIndex < buffer.byteLength) {
    // Get Total GRIB length from buffer
    let gribLength = new DataView(
      buffer.slice(gribByteIndex + 12, gribByteIndex + 16)
    ).getInt32(0);
    gribFileBuffers.push(
      buffer.slice(gribByteIndex, gribByteIndex + gribLength)
    );
...
export const getImgElement = function (data: any) {
  ...
  for (let i = 0; i < imgData.data.length; i += 4) {
    ...
    let alpha =
      data.values[i / 4] === null || isNaN(data.values[i / 4]) ? 0 : 255;
...
const decodeGRIB2Buffer = function (buffer: ArrayBuffer, myGrib: GRIB2) {
  ...
  if (myGrib.dataTemplate[0][3].content !== 2) {
    console.error(
      "This is not a GRIB2. File edition number: " +
        myGrib.dataTemplate[0][3].content
    );
    let str = "** Section 0 <info>: <content> ** \n";
    for (let i = 0; i < myGrib.dataTemplate[0].length; i++)
      str +=
        myGrib.dataTemplate[0][i].info +
        ": " +
        myGrib.dataTemplate[0][i].content +
        "\n";
    console.error(str);
    return;
  }
  ...
  // Discipline
  console.log(
    myGrib.dataTemplate[0][2].info +
      ": " +
      myGrib.dataTemplate[0][2].contentRef +
      "(" +
      myGrib.dataTemplate[0][2].content +
      ")"
  );
  ...
  while (sectionByteIndex < buffer.byteLength) {
    // Get Total GRIB length from buffer
    let sectionLength = new DataView(
      buffer.slice(sectionByteIndex, sectionByteIndex + 4)
    ).getInt32(0);
    sectionBuffers.push(
      buffer.slice(sectionByteIndex, sectionByteIndex + sectionLength)
    );
  ...
  if (sectionBuffers.length > 8)
    console.error(
      "TODO: implement repeated sections in grib buffer. Number of sections inside grib buffer: " +
        sectionBuffers.length
    );

  // Decode section buffers
  let usedSectionNumbers = [];
  for (let i = 0; i < sectionBuffers.length - 1; i++) {
    // (length - 1) because of end section has 4 bytes only (7777)
    let sectionNumber = new DataView(sectionBuffers[i].slice(4, 5)).getInt8(0);
    myGrib.dataTemplate[sectionNumber] = decodeSection(
      sectionBuffers[i],
      myGrib.dataTemplate[sectionNumber]
    );
    ...
    if (usedSectionNumbers.includes(sectionNumber))
      console.error(
        "TODO: section " +
          sectionNumber +
          "repeated: sections can be repeated inside GRIB. Right now only using the last repeated section"
      );
    usedSectionNumbers.push(sectionNumber);
  }
  // Section 8 data template does not contain the section number
  myGrib.dataTemplate[8] = decodeSection(
    sectionBuffers[sectionBuffers.length - 1],
    myGrib.dataTemplate[8]
  );
...
const parseData = function (decodedGrib) {
  ...
  grid.numLongPoints = getContentByInfo(
    decodedGrib[3],
    "Ni — number of points along a parallel"
  );
  grid.numLatPoints = getContentByInfo(
    decodedGrib[3],
  ...
  grid.incJ =
    getContentByInfo(
      decodedGrib[3],
      "Dj — j direction increment (see Note 1 and 5)"
    ) / 1e6;

  grid.scanningMode = getContentRefByInfo(
    decodedGrib[3],
    "Scanning mode (flags — see Flag Table 3.4 and Note 6)"
  );

  // Check if numPoints corresponds to numLatPoints * numLongPoints
  if (grid.numPoints != grid.numLatPoints * grid.numLongPoints)
    console.warn(
      "Section 3. Grid numPoints does not equal to numLatPoints * numLongPoints. numPoints = " +
        grid.numPoints +
        " , numLatPoints * numLongPoints = " +
        grid.numLatPoints * grid.numLongPoints
    );

  // Check if the number of values
  let numValues = getContentByInfo(
    decodedGrib[5],
  ...
  if (numValues !== grid.numPoints)
    console.warn(
      "Num values in Section 7: " +
        numValues +
        ", Grid points in Section 5: " +
        grid.numPoints +
        ". Probably bitmap is present"
    );
  ...
  if (decodedGrib[0][2].content != 255) {
    // 255: Missing
    ...
    let parameterCategoryTable =
      GRIB2.tables["4.2-" + disciplineNum + "-" + paramCat];
    if (parameterCategoryTable == undefined) {
      console.warn(
        "GRIB2 table 4.2-" + disciplineNum + "-" + paramCat + " not defined."
      );
    } else product[paramNumInfo] = parameterCategoryTable[paramNum];
  } else {
    console.warn(
      "Discipline not defined, therefore no interpretation of product category, paramater, and units."
    );
  }
  ...
  if (
    ...
    let compression = {};
    compression.refValue = getContentByInfo(
      decodedGrib[5],
      "Reference value (R) (IEEE 32-bit floating-point value)"
    );
    compression.binaryScaleFactor = getContentByInfo(
      decodedGrib[5],
      "Binary scale factor (E)"
    );
    compression.decimalScaleFactor = getContentByInfo(
      decodedGrib[5],
      "Decimal scale factor (D)"
    );
    compression.bitsPerValue = getContentByInfo(
      decodedGrib[5],
  ...
  if (template == "5.0") {
    ...
    console.log(
      "Bit length/bits per point : " +
        (rawData.byteLength * 8) / compression.bitsPerValue +
        ", Num points: " +
        grid.numPoints
    );
    // Read bits
    values = readValuesFromBuffer(
      rawData,
  ...
  // https://www.nco.ncep.noaa.gov/pmb/docs/grib2/grib2_doc/grib2_temp5-2.shtml
    ...
    let X1 = readValuesFromBuffer(
      rawData.slice(0, xxIndex),
      compression.bitsPerValue,
      NG
    );
    ...
    let bitsPerValueGroupWidth = getContentByInfo(
      decodedGrib[5],
      "Number of bits used for the group widths (after the reference value in octet 36 has been removed"
    );
    let groupWidths = readValuesFromBuffer(
      rawData.slice(xxIndex, yyIndex),
      bitsPerValueGroupWidth,
      NG
    );

    let zzIndex = yyIndex + xxIndex;
    // Scaled group lengths
    let bitsPerValueScaledGL = getContentByInfo(
      decodedGrib[5],
    ...
    let scaledGroupLengths = readValuesFromBuffer(
      rawData.slice(yyIndex, zzIndex),
      bitsPerValueScaledGL,
      NG
    );

    // Group lengths (The group length (L) is the number of values in a group.)
    let groupLengths = [];
    let refGroupLength = getContentByInfo(
      decodedGrib[5],
      "Reference for group lengths (see Note 13)"
    );
    let lengthInc = getContentByInfo(
      decodedGrib[5],
    ...
    for (let i = 0; i < NG; i++) {
      ...
      let numBytes = Math.ceil(
        (numValuesPerGroup * bitsPerValuePerGroup + lastBitIndex) / 8
      ); // Total num bits + previous shift amount
      let X2PerGroup = readValuesFromBuffer(
        rawData.slice(lastByteIndex, lastByteIndex + numBytes),
        bitsPerValuePerGroup,
        numValuesPerGroup,
        lastBitIndex
      ); // Because X2 are consecutive, we keep lastBitIndex
      lastBitIndex =
        (numValuesPerGroup * bitsPerValuePerGroup + lastBitIndex) % 8; // LastBitIndex goes from 0 to 7, as every iteration we send the specific bytes to read.
  ...
  // https://apps.ecmwf.int/codes/grib/format/grib2/regulations/
    ...
    let diffOrder = getContentByInfo(
      decodedGrib[5],
      "Order of spatial difference (see Code Table 5.6)"
    ); // WARNING --> IF CODE TABLE IS USED, THIS IS A STRING. OTHERWISE 1= 1ST ORDER, 2= SECOND ORDER
    // Field width (Number of bytes for the first values)
    let fieldWidth = getContentByInfo(
      decodedGrib[5],
    ...
    if (fieldWidth == 1)
      ...
      console.error(
        "Field width is not reasonable (in parseData, template 5.3)"
      );
    ...
    // Regulation: '92.1.5'
    let strBits = bytes2bits(
      new Uint8Array(
        rawData.slice(fieldWidth * diffOrder, fieldWidth * (diffOrder + 1))
      )
    ); // Get string of bits
    ...
    let unpackedArray = unpackComplexPacking(
      decodedGrib,
      rawData,
      compression,
      wwIndex
    ...
    if (grid.numPoints != f.length)
      console.error(
        "Length of raw datapoints (H) is not equal to grid points : grid.numPoints: " +
          grid.numPoints +
          ", h.length: " +
          h.length
      );
  ...
  // https://www.nco.ncep.noaa.gov/pmb/docs/grib2/grib2_doc/grib2_temp5-4.shtml
    ...
    let dataType = getContentRefByInfo(
      decodedGrib[5],
      "Precision (See code Table 5.7)"
    ); //decodedGrib[5][4].contentRef;
    let byteSize;
    let type;
    ...
    for (let i = 0; i < numValues; i++) {
      ...
      values[i] = decodeByte(
        rawData.slice(i * byteSize, i * byteSize + byteSize),
        type
      );
    }
  }
  ...
  if (decodedGrib[6][2].content !== 255) {
    console.warn(
      "Bitmap is present: " +
        getContentRefByInfo(
          decodedGrib[6],
          "Bit-map indicator (See Table 6.0) (See note 1 below)"
        )
    ); //decodedGrib[6][2].contentRef)
    // Bitmap
    if (decodedGrib[6][2].content == 0) {
      ...
      for (let i = 0; i < grid.numPoints; i++) {
        // With bitmap
        if (data.bitmap[i] == 0) griddedValues[i] = null; // Empty value
        else {
      ...
      console.error(
        "TODO: bitmap is defined in a previous dataset in the same grib file."
      );
    }
  }
  ...
  if (grid.scanningMode[0][0] == 1) {
    // Points in the first row or column scan in the -i (-x) direction
    console.error(
      "TODO: Scanning mode: the data needs to be mirroed in the vertical axis (east should be west and viceversa)"
    );
  }
  ...
  if (grid.scanningMode[1][0] == 0) {
    ...
    if (grid.latStart < grid.latEnd) {
      // Grid defines the first grid latitude point as the southest
      ...
      for (let i = 0; i < data.values.length; i++) {
        ...
        let flippedIndex =
          (grid.numLatPoints - rowN - 1) * grid.numLongPoints + colN;
  ...
  if (grid.scanningMode[2][0] == 1)
    console.error(
      "TODO: Scanning mode: Adjacent points in the j (y) direction are consecutive"
    );
  if (grid.scanningMode[3][0] == 1)
    console.error(
      "TODO: Scanning mode: Adjacent rows scan in the opposite direction"
    );
  if (grid.scanningMode[4][0] == 1)
    console.error(
      "TODO: Scanning mode: Points within odd rows are offset by Di/2 in i(x) direction"
    ); // Making a grid with diamonds and not squares
  if (grid.scanningMode[5][0] == 1)
    console.error(
      "TODO: Scanning mode: Points within even rows are offset by Di/2 in i(x) direction"
    ); // Making a grid with diamonds and not squares
  ...
  if (grid.scanningMode[7][0] == 1)
    console.error(
      "TODO: Scanning mode: 8th bit at https://www.nco.ncep.noaa.gov/pmb/docs/grib2/grib2_doc/grib2_table3-4.shtml"
    );
...
const unpackComplexPacking = function (
  decodedGrib,
  rawData,
  compression,
  inwwIndex
  ...
  let refGroupWidth = getContentByInfo(
    decodedGrib[5],
  ...
  for (let i = 0; i < NG; i++) groupWidths[i] += refGroupWidth;
  ...
  // encoded as 0.
  let groupSplittingMethod = getContentByInfo(
    decodedGrib[5],
    "Group splitting method used (see Code Table 5.4)"
  );
  if (groupSplittingMethod != 1)
    console.error(
      "Group splitting method not implemented. See: https://www.nco.ncep.noaa.gov/pmb/docs/grib2/grib2_doc/grib2_table5-4.shtml"
    );
  ...
  let lengthLastGroup = getContentByInfo(
    decodedGrib[5],
    "True length of last group"
  );
  if (lengthLastGroup != groupLengths[groupLengths.length - 1]) {
    // Double check
    console.warn(
      "True length of last group is not the same as the last value in the gruop length array. " +
        lengthLastGroup +
        " vs " +
        groupLengths[groupLengths.length - 1]
    );
  ...
  let section3NumPoints = getContentByInfo(
    decodedGrib[3],
    "Number of data points"
  );
  if (totalNumPoints != section3NumPoints)
    console.warn(
      "Total number of points: " +
        totalNumPoints +
        ", Section 3 num of points: " +
        section3NumPoints
    );
  if (
    totalNumBits / 8 - rawData.slice(zzIndex, rawData.byteLength).byteLength >
    1
  )
    console.warn(
      "Total number of required bytes: " +
        totalNumBits / 8 +
        ", Available bytes: " +
        rawData.slice(zzIndex, rawData.byteLength).byteLength
    );

  // Substitute values for missing data
  let primarySubstitute = getContentByInfo(
    decodedGrib[5],
    "Primary missing value substitute"
  );
  let secondarySubstitute = getContentByInfo(
    decodedGrib[5],
  ...
  for (let i = 0; i < NG; i++) {
    ...
    if (i == NG - 1 && lastByteIndex - rawData.byteLength > 1)
      console.warn(
        "LastByteIndex: " +
          lastByteIndex +
          ". Available bytes: " +
...
const readValuesFromBuffer = function (
  arraybuffer,
  bitsPerValue,
  numValues,
  inBitIndex
  ...
  if ((bitsPerValue * numValues + bitIndex) / 8 > rawUint8.length)
    console.error(
      "Num required bytes: " +
        (bitsPerValue * numValues + bitIndex) / 8 +
        ", Num available bytes: " +
        rawUint8.length
    );
...
const getContentByInfo = function (sectionArray, infoValue) {
  for (let i = 0; i < sectionArray.length; i++) {
    if (sectionArray[i].info === infoValue) return sectionArray[i].content;
  }
};
// Get contentRef by infoValue
const getContentRefByInfo = function (sectionArray, infoValue) {
  for (let i = 0; i < sectionArray.length; i++) {
    if (sectionArray[i].info === infoValue) return sectionArray[i].contentRef;
  }
};

// Decode section
const decodeSection = function (buffer: ArrayBuffer, section) {
  for (let i = 0; i < section.length; i++) {
    ...
    if ("templateRef" in prop) {
      ...
      try {
        section = section.concat(
          JSON.parse(JSON.stringify(GRIB2.templates[templateId]))
        ); // Copy template
      } catch (error) {
    ...
    // Decode bytes
    let byteContentBuffer = buffer.slice(
      prop.startIndex - 1,
      prop.startIndex - 1 + prop.size
    );
...
function decodeByte(byteContent, type: DTypes) {
  let content;

  if (byteContent.byteLength == 0) content = null;
  else if (type === "String")
    content = String.fromCharCode.apply(null, new Uint8Array(byteContent));
  else if (type == "uint8") content = new DataView(byteContent).getUint8(0);
  else if (type == "uint16") content = new DataView(byteContent).getUint16(0);
  else if (type == "uint32") content = new DataView(byteContent).getUint32(0);
  else if (type == "uint64")
    content = Number(new DataView(byteContent).getBigUint64(0));
  else if (type == "int8") content = new DataView(byteContent).getInt8(0);
  else if (type == "int16") content = new DataView(byteContent).getInt16(0);
  else if (type == "int32") content = new DataView(byteContent).getInt32(0);
  else if (type == "int64")
    content = Number(new DataView(byteContent).getBigInt64(0));
  else if (type == "float32") content = new DataView(byteContent).getFloat32(0);
  else if (type == "float64") content = new DataView(byteContent).getFloat64(0);
  else if (type == "float128") {
    console.error("TODO: float128 parsing is not implemented");
    content = new DataView(byteContent).getFloat64(0);
  } else content = byteContent;
...
function bytes2bits(bytes) {
  ...
  for (let i = 0; i < bytes.length; i++) {
    let byte = bytes[i];
    for (let j = 128; j >= 1; j /= 2) bits += byte & j ? "1" : "0";
  }
  return bits;
}
